<template>
  <div>
  	...
  	<el-table  :data='tableData' highlight-current-row>
			...
			<el-table-column prop="target.account.name" label="作者"></el-table-column>
			...
			<el-table-column prop="target.updated" label="发布时间" min-width="95" sortable></el-table-column>
...
<script>
  ...
  export default {
    ...
    methods:{
    	...
    	getReportList(t){
      	newsApi.news.reportList(t).then(res => {
            ...
            this.tableData[i].target.location = this.tableData[i].target.location.province + '·' + this.tableData[i].target.location.city;
      ...
      handleCurrentChange(val) {
          ...
          for(let i = 0; i < this.tableData.length; i++){
            ...
            for(let j = 0; j < stateLists.length; j++){
              if(this.tableData[i].target.type == j){
                this.tableData[i].target.ype = stateLists[j];
              }
            }
          }
        })
    ...
    mounted() {
      const filterdata = { index:'', filter:{limit:10, states:["0"], offset: (this.page - 1) * 10,targetType:'2' }};  
