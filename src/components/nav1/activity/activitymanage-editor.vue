    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="130px" label="right" class="publishForm">
	    ...
	    <el-form-item style="width:986px;" v-for="(guest, index) in guestLists" :key="guest">
	    	<el-col :span="24" style="border:1px solid #ccc;">
		    	...
		    	<el-col :span="16">
		    		<el-row>
		    			<el-col :span="7">姓名：{{guest.name}}</el-col>
		    			<el-col :span="7" :offset="1">公司：{{guest.company}}</el-col>
		    			<el-col :span="7" :offset="1">职位：{{guest.job}}</el-col>
		    		</el-row>
		    		<el-row>
		    			<el-col :span="23">简介：{{guest.info}}</el-col>
		    		</el-row>
		    	</el-col>
		    	<el-col :span="3">
	    ...
	    <el-form-item style="width:986px;" v-for="(item, index) in processLists" :key="item">
	    	<el-row style="border-bottom:1px dashed #ccc;">
	    		...
	    		<el-col :span="2" :offset="1">
	    			<el-button type="primary" size="small" @click="modifyProcess(index)">修改</el-button>
	    		</el-col>
	    		<el-col :span="2">
	    			<el-button type="primary" size="small" @click="deleteProcess(index)">删除</el-button>
	    		</el-col>
	    	</el-row>
	    ...
	    <el-form-item label="活动类型:" prop="cost" required>
			...
			<el-form-item>
				<el-button type="primary" @click.prevent="onSubmit" v-if="state!='0'">保存</el-button>
				<el-button type="primary" @click.prevent="onSubmit" v-if="state=='0'">马上发布</el-button>
				<el-button @click.prevent="onSubmit" v-if="state=='0'">保存草稿</el-button>
  ...
  export default {
    ...
    data() {
    	var validateDate = (rule, value, callback) => {
    		var startOfActivity = functionApi.moment.toGMTFormate(functionApi.moment.getDateFormate(this.form.activityStartTimeDate) + ' ' + functionApi.moment.getHM(this.form.activityStartTimeTime));
      ...
      clearthumb(imgSrc){
        var vm = this;
        vm.form[imgSrc] = '';
      	document.getElementById('guestAvatar0').value = '';
      },
      choosePoster(index){
				for(let i = 0; i < 16; i++){
      ...
      addGuest(){
      	var vm = this;
	      if(vm.form.guestName!=='' && vm.form.guestCompany!=='' && vm.form.guestPosition!=='' && vm.form.guestInfo!==''){
	        var list = {
      ...
      onSubmit(e) {
      	...
      	// vm.temp.content = vm.temp.content.substring(8);
      	// vm.form.content = vm.form.content.split('<article>')[1].split('</article>')[0];
        let postdata = {
        	id: localStorage.getItem('activityId'),
          title: vm.form.title,
          location: {province:vm.form.location[0], city:vm.form.location[1], area: vm.form.address},
        	startOfActivity: functionApi.moment.toGMTFormate(functionApi.moment.getDateFormate(vm.form.activityStartTimeDate) + ' ' + functionApi.moment.getHM(vm.form.activityStartTimeTime)),
        	endOfActivity: functionApi.moment.toGMTFormate(functionApi.moment.getDateFormate(vm.form.activityEndTimeDate) + ' ' + functionApi.moment.getHM(vm.form.activityEndTimeTime)),
        	...
        	numberOfExpected: vm.form.num == 0 ? -1 : vm.form.num,
        ...
        if(option == '保存'){
        	this.$refs['form'].validate((valid) => {
	            ...
	            axios.post(this.uploadUrl + '?action=uploadHTML', formData).then(res => {
	              postdata.content = this.activityData.url;
		...
		mounted() {
          ...
          self.setContent(html, text);
			...
			/*	if(localStorage.getItem('previewData')){
					this.activityData = JSON.parse(localStorage.getItem('previewData'));
...
<style lang="scss" scoped>
	.publishForm{ margin:20px; width:60%; min-width:760px; 
