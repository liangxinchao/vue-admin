<template>
  <div>
    <el-form :inline="true">
      ...
      <el-table-column prop="ActivityTime" label="活动时间" width="155" sortable></el-table-column> 
      <el-table-column prop="created" label="发布时间" width="155" sortable></el-table-column>
      <el-table-column prop="state" label="状态"></el-table-column>
      <el-table-column label="操作" width="146">
        <template scope="scope">
          <el-button size="small" :disabled="scope.row.state!='报名中' && scope.row.state!='预热中' && scope.row.state!='进行中'" @click="handlePush(scope.row)">推送</el-button>
	...
	export default {
      ...
      export2Excel: function(){
      	...
      	for(let i = 0; i < ids.length; i++){
	        ...
	        export_json_to_excel(tHeader, data, '活动列表');
	      })
      },
			getActivityList(t){
