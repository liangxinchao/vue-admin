<template>
  <div>
    <el-form :inline="true">
      ...
      <el-table-column prop="created" label="发表时间"></el-table-column>
      <el-table-column prop="created" label="下架时间"></el-table-column>
...
<script>
  ...
  import functionApi from '@/js/function.js';
	export default {
        ...
        this.getReplyList(this.filterdata);
			},
			filterDateBtn() {
        this.filterdata.index = this.pageindex;
      ...
      selsChange(sels) {
        this.sels = sels;
			...
			handleEdit(index, row) {
				localStorage.setItem('replyId', row.id);
				this.$router.push({path:'/nav1/dynamic/replylist/manage'})
  		},
