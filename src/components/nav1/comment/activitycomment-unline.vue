<template>
  <div>
    ...
    <el-table :data='tableData' highlight-current-row @selection-change="selsChange">
      ...
      <el-table-column prop="relative" label="关联活动"></el-table-column>
      <el-table-column prop="data" label="回复/点赞"></el-table-column>
    ...
    <el-col :span="24" class="toolbar">
      <el-button type="success" @click="" :disabled="this.sels.length===0">恢复</el-button>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      getTableList(t){
        this.tableData = [
          {content:'评论11111111', author:'张三', relative:'梦想小镇周年庆', data:'12/20', create:'2017-03-26'}
        ]
      },
      ...
      handleEdit(index, row) {
        localStorage.setItem('activitycommentId', row.id);
        this.$router.push({path:'/nav1/comment/activity/manage'});
      }
    },
    mounted() {
      const filterdata = {
