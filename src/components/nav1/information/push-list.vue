  ...
  <div>
    <el-form :inline="true">
      ...
      <el-form-item class="label-sel">
        <el-select v-model="pushState" placeholder="请选择" @change="filterBtn">
    ...
    <el-table :data='tableData' highlight-current-row>
      ...
      <el-table-column prop="content" label="推送内容" width="260"></el-table-column>
    ...
    methods:{
      ...
      handleReSend(row){
        this.$confirm('确定要重发?', '提示', {
          ...
          infoApi.info.pushRadioReSend({id: row.id}).then(res => {
      ...
      withdraw(row){
        this.$confirm('确定要撤回, 是否继续?', '提示', {
          ...
          infoApi.info.pushRadioWithdraw({id:row.id}).then(res => {
      ...
      handleDetail(row){
        localStorage.setItem('pushId',row.id);
        this.$router.push({path:'/nav1/information/push/detail'})
      },
      filterBtn(){
      ...
      filterDateBtn(){
        this.filterdatastart = functionApi.moment.getListTime(this.dateTime[0]);
        this.filterdataend = functionApi.moment.getListTime(this.dateTime[1]);
      },
      getTableList(t){
        infoApi.info.pushRadioList(t).then(res => {
          this.tableData = res.data.redioList;
